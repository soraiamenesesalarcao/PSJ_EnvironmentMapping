...
void TGA_Texture(unsigned int *textureArray, char *strFileName, int ID) {
	...
	//Com a rotina abaixo associa-se o objecto textura ao texture unit que est‡ activo
	glBindTexture(GL_TEXTURE_2D, textureArray[ID]);

	textureType = GL_RGB;
	if(pBitMap->channels == 4)	textureType = GL_RGBA;
	
	glTexImage2D(GL_TEXTURE_2D,0, pBitMap->channels, pBitMap->size_x, pBitMap->size_y,
